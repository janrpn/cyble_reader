...
uint8_t Frequency0 = 0x75;
uint8_t Frequency1 = 0xAF;
uint8_t Frequency2 = 0x10;
float Mhz = 433.82;
int pa=10;
int last_pa=-1;
int modulation = 0;
...
halRfWriteReg(FREQ2,Frequency2);   //Frequency Control Word, High Byte  Base frequency = 433.82
halRfWriteReg(FREQ1,Frequency1);   //Frequency Control Word, Middle Byte
halRfWriteReg(FREQ0, Frequency0);   //rasmobo 810 819.5 OK
...
void thermostat_On()
{
    ...
    Mhz = 868.3;
    cc1101_init();
    echo_debug(debug_out,"Sending thermostat on");
    
    uint8_t m_txTurnOn[11] = {0x00, 0xAA, 0xAA, 0xAA, 0x2D, 0xD4, 0xF8, 0x1, 0xF9, 0x7B, 0x00, }; 
    ...
    SPIWriteBurstReg(TX_FIFO_ADDR,m_txTurnOn,sizeof(m_txTurnOn));
...
uint8_t scenario_releve(void)
{
	...
	if (debug_out)
    {
        echo_debug(debug_out,"prepared send message: %d \r\n", msgSize);
        show_in_hex(&txbuffer[0], msgSize);
    }
